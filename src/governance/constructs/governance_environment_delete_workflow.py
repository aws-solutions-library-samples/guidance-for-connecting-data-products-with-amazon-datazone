...
class GovernanceManageEnvironmentDeleteWorkflowConstruct(Construct):
    """ Class to represent the workflow that will execute after a Amazon DataZone environment is triggered for deletion.
    The workflow will clean environment roles' permission boundaries and policies to their original state.
    ...
    def __init__(self, scope: Construct, construct_id: str, governance_props: dict, workflow_props: dict, common_constructs: dict, env: Environment, **kwargs) -> None:
        ...
        # ---------------- Step Functions ------------------------
        g_manage_environment_delete_state_machine_name = 'dz_conn_g_manage_environment_delete'
        
        g_manage_environment_delete_state_machine_logs = logs.LogGroup(
            scope= self,
            id= 'g_manage_environment_active_state_machine_logs',
            log_group_name=f'/aws/step-functions/{g_manage_environment_delete_state_machine_name}',
            removal_policy=RemovalPolicy.DESTROY
        )

        g_manage_environment_delete_state_machine = stepfunctions.StateMachine(
            scope= self,
            id= 'g_manage_environment_delete_state_machine',
            state_machine_name= g_manage_environment_delete_state_machine_name,
            definition_body=stepfunctions.DefinitionBody.from_file('src/governance/code/stepfunctions/governance_manage_environment_delete_workflow.asl.json'),
            definition_substitutions= {
                'g_get_environment_details_lambda_arn': common_constructs['g_get_environment_details_lambda'].function_arn,
                'a_clean_environment_roles_lambda_name': GLOBAL_VARIABLES['account']['a_clean_environment_roles_lambda_name'],
            ...
            logs= stepfunctions.LogOptions(
                destination=g_manage_environment_delete_state_machine_logs,
        ...
        g_manage_environment_delete_rule = events.Rule(
            scope= self,
            id= 'g_manage_environment_delete_rule',
            rule_name= 'dz_conn_g_manage_environment_delete_rule',
        ...
        g_manage_environment_delete_rule_target = event_targets.SfnStateMachine(
            machine= g_manage_environment_delete_state_machine,
        ...
        g_manage_environment_delete_rule.add_target(g_manage_environment_delete_rule_target)
