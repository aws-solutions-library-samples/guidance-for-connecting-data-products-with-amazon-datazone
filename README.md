...
1. A producer needs to create a project that will host and publish the data asset in Amazon DataZone service.
2. A producer needs to map the data asset into a AWS Glue database. Note that it is recommended to use the AWS Glue database associated to the producer's Amazon DataZone project.
3. The producer needs to create / execute a publishing job in Amazon DataZone that publishes the data asset into the Amazon DataZone common business catalog.
4. A consumer needs to create a project that will be used to subscribe and consume the published data asset.
5. A consumer needs to submit a subscription request (with its justification) to be granted access to the published data asset. This is done from within the consumer's Amazon DataZone project.
6. A producer needs to accept the subscription request, so that access to the data asset is granted to the consumer.
...
1. An [AWS Service Catalog](https://aws.amazon.com/servicecatalog/) portfolio hosting a complementary toolkit that can be used in a self service manner, which allow users (producers and consumers) to:
    ...
    - Map source data asset hosted in a JDBC source into a target AWS Glue database. (Activity 2)
    - Consume / query a published data asset hosted in a JDBC source after access is granted. (Activity 7)

2. A set of automated workflows that will trigger on certain Amazon DataZone events like:
    
    1.  When a new Amazon DataZones is successfully created so that default project capabilities are extended to support this solution complimentary toolkit,described above. (Activities 1 and 4)
    2.  When a subscription request to a published data asset is accepted so that actual access is granted automatically to the corresponding consumer project. (Activity 6)
    3. When a subscription to a published data asset is revoked so that actual access is revoked automatically from the corresponding project.
    4. When an existing Amazon DataZone project deletion starts, so that non-default Amazon DataZone capabilities are removed from project.

Note that activities 3 and 5 don't need any complementary task to be executed (automatically nor manually).

The following diagram is the solution's reference architecture illustrating the workflow 2.2 for when a subscription request is approved, described above (which you can consider as the core functionality of this solution). It also illustrates how tools in the solutions toolkit are leveraged by producers and consumers before and after workflow execution. 

![this solution reference architecture](assets/images/reference-architecture.png)

## How to deploy this solution?

This solution uses [AWS Cloud Development Kit (CDK)](https://aws.amazon.com/cdk/) CLI to automate its deployment. It contains two applications; the first application is intended to be deployed in the central governance account (where Amazon DataZone service is configured) and second application is intended to be deployed in each of the governed accounts (where data sources are hosted and from where producer / consumer projects will be used). 
...
### Deploy prerequisites in all governed accounts

In order to deploy solution's resources in each of the governed accounts; meaning accounts where Amazon DataZone projects will be hosted, it is necessary to execute the following steps in each one of them:

#### Bootstrap governed accounts

In order to bootstrap the governed account you need to have an AWS CLI profile for each governed account already in place. For more details please review the AWS documentation.

Run the following CDK command for each governed account to allocate base CDK resources (known as bootstrapping) in each one of them. Replace ```<PROFILE_NAME>``` with the corresponding profile name that maps to the governed account you are going to bootstrap.

``` sh
cdk bootstrap --profile <PROFILE_NAME>
```

As part of the bootstrapping process, CDK will create a set of Amazon IAM roles that will be used when deploying resources in your governed accounts. These can be accessed through the AWS console or using the AWS CLI.

Take note of the name of the new CDK AWS CloudFormation execution role that was created in each of your governed accounts. The role name should follow the structure *cdk-<RANDOM_VALUE>-cfn-exec-role-<ACCOUNT_ID>-<AWS_REGION>*.
...
- Number of project related workflow (creation / deletion) executions in a month:  20
